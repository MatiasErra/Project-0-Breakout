...
function LevelMaker.createMap(level)
 ...
 local blockedBrickCount =  1--math.min(5, math.floor( math.random(level)/2))
    ...
    if #bricks == 0 then
       ...
       -- if blockedBrickLevel then 
            while blockedBrickCount > 0 do 
                local countBricks = #bricks
                local randomBlocked = math.random(1, countBricks)

                for i, brick in pairs(bricks) do 
                    if i == randomBlocked then 
                        brick.blocked = true 
                        blockedBrickCount = blockedBrickCount -1 
                    end
                end 
            end
      --  end 
